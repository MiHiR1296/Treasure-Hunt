-- Puzzle Chain System Migration (SAFE VERSION - No DROP statements)
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'puzzle_steps' 
    AND policyname = 'Puzzle steps are viewable by everyone'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'puzzle_steps' 
    AND policyname = 'Anyone can create puzzle steps'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'puzzle_steps' 
    AND policyname = 'Anyone can update puzzle steps'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'puzzle_steps' 
    AND policyname = 'Anyone can delete puzzle steps'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'puzzle_progress' 
    AND policyname = 'Teams can view their own puzzle progress'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'puzzle_progress' 
    AND policyname = 'Teams can insert their own puzzle progress'
  ) THEN
