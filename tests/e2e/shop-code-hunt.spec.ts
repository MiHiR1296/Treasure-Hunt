import { expect, test } from '@playwright/test';
import { createInitialState, executeCommand, getPlayerView } from '../../lib/engine';
import { huntTemplates } from '../../lib/engine/templates';

const definition = structuredClone(huntTemplates.find(template => template.id === 'frankie-code-hunt')!.definition);
const state = createInitialState(definition, 'frankie-browser-team', '2026-09-22T10:00:00.000Z');
const view = {
  ...getPlayerView(definition, state, '2026-09-22T10:00:10.000Z'),
  teamName: 'Wrap Raiders', members: ['Mira', 'Dev'], isPreview: false, eventStatus: 'live' as const,
};
let progressedState = executeCommand(definition, state, { type: 'verify', checkpointId: 'name-the-snack', nodeId: 'answer', value: 'roll' }, '2026-09-22T10:00:20.000Z').state;
progressedState = executeCommand(definition, progressedState, { type: 'continue', checkpointId: 'name-the-snack', nodeId: 'word' }, '2026-09-22T10:00:21.000Z').state;
const progressedView = { ...getPlayerView(definition, progressedState, '2026-09-22T10:00:22.000Z'), teamName: 'Wrap Raiders', members: ['Mira', 'Dev'], isPreview: false, eventStatus: 'live' as const };
let ingredientState = executeCommand(definition, progressedState, { type: 'submit_puzzle', checkpointId: 'frankie-picture', nodeId: 'puzzle', expectedRevision: 0, value: { order: ['mint', 'copper', 'sesame', 'amber', 'paneer', 'chilli', 'onion', 'roti', 'plate'] } }, '2026-09-22T10:01:00.000Z').state;
ingredientState = executeCommand(definition, ingredientState, { type: 'continue', checkpointId: 'frankie-picture', nodeId: 'word' }, '2026-09-22T10:01:01.000Z').state;
for (const [expectedRevision, path] of [
  Array.from({ length: 6 }, (_, index) => ({ row: 5 - index, column: 1 })),
  Array.from({ length: 7 }, (_, index) => ({ row: 2, column: 7 - index })),
  Array.from({ length: 6 }, (_, index) => ({ row: index + 2, column: 7 })),
].entries()) ingredientState = executeCommand(definition, ingredientState, { type: 'submit_puzzle', checkpointId: 'ingredient-search', nodeId: 'puzzle', expectedRevision, value: { path } }, '2026-09-22T10:01:02.000Z').state;
const ingredientView = { ...getPlayerView(definition, ingredientState, '2026-09-22T10:01:03.000Z'), teamName: 'Wrap Raiders', members: ['Mira', 'Dev'], isPreview: false, eventStatus: 'live' as const };

test('Frankie player shows one direct question with dot progress and no repeated interface labels', async ({ page }, testInfo) => {
  await page.route('**/api/v2/session', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ view }) }));
  await page.route('**/api/v2/hunts', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ hunts: [{ id: definition.id, title: definition.title }] }) }));
  await page.route('**/api/v2/leaderboard**', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ visible: true, entries: [] }) }));
  await page.goto(`/v2?hunt=${definition.id}`);

  await expect(page.getByRole('heading', { name: 'Name the snack' })).toBeVisible();
  await expect(page.getByRole('list', { name: 'Hunt progress' })).toBeVisible();
  await expect(page.getByRole('list', { name: 'Hunt progress' }).locator('[aria-current="step"]')).toHaveCount(1);
  await expect(page.getByText(/Mumbai’s grab-and-go bite/)).toBeVisible();
  await expect(page.getByLabel('Your answer')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Check answer', exact: true })).toBeVisible();
  await expect(page.getByText('Checkpoint 1', { exact: true })).toHaveCount(0);
  await expect(page.getByText('Read this clue', { exact: true })).toHaveCount(0);
  await expect(page.getByText(/^Round 1/)).toHaveCount(0);
  await expect(page.getByText(/Accepted answers:/)).toHaveCount(0);
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth + 1)).toBeTruthy();
  if (testInfo.project.name === 'android-chrome') await page.screenshot({ path: '.data/verification/frankie-code-hunt-player.png', fullPage: true, animations: 'disabled' });
  await testInfo.attach('Frankie player mobile', { body: await page.screenshot({ fullPage: true, animations: 'disabled' }), contentType: 'image/png' });
});

test('players can revisit completed stages without seeing future riddles', async ({ page }, testInfo) => {
  await page.route('**/api/v2/session', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ view: progressedView }) }));
  await page.route('**/api/v2/hunts', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ hunts: [{ id: definition.id, title: definition.title }] }) }));
  await page.route('**/api/v2/leaderboard**', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ visible: true, entries: [] }) }));
  await page.goto(`/v2?hunt=${definition.id}`);

  await expect(page.getByRole('heading', { name: 'Rebuild the Frankie' })).toBeVisible();
  await page.getByRole('button', { name: 'Name the snack, completed stage' }).click();
  await expect(page.getByRole('heading', { name: 'Name the snack' })).toBeVisible();
  await expect(page.getByText(/Mumbai’s grab-and-go bite/)).toBeVisible();
  await expect(page.getByText('Your result: Frankie, roll, or wrap', { exact: true })).toBeVisible();
  await expect(page.getByText('🔓 GOOD', { exact: false })).toBeVisible();
  await expect(page.getByText(/Tall, cold, and full of ingredients/)).toHaveCount(0);
  if (testInfo.project.name === 'android-chrome') await page.screenshot({ path: '.data/verification/frankie-stage-history.png', fullPage: true, animations: 'disabled' });
  await page.getByRole('button', { name: 'Back to the current challenge' }).click();
  await expect(page.getByRole('heading', { name: 'Rebuild the Frankie' })).toBeVisible();
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth + 1)).toBeTruthy();
});

test('ingredient search explains its minimum and keeps bonus play optional on mobile', async ({ page }, testInfo) => {
  await page.route('**/api/v2/session', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ view: ingredientView }) }));
  await page.route('**/api/v2/hunts', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ hunts: [{ id: definition.id, title: definition.title }] }) }));
  await page.route('**/api/v2/leaderboard**', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ visible: true, entries: [] }) }));
  await page.goto(`/v2?hunt=${definition.id}`);

  await expect(page.getByRole('heading', { name: 'Find the ingredients' })).toBeVisible();
  await expect(page.getByText('Find at least 3 of 8 ingredients.')).toBeVisible();
  await expect(page.getByText(/Each extra ingredient is worth \+2 points/)).toBeVisible();
  await expect(page.getByRole('button', { name: 'Continue with 3 ingredients' })).toBeVisible();
  await expect(page.getByText('✓ PANEER found')).toBeVisible();
  if (testInfo.project.name === 'android-chrome') await page.screenshot({ path: '.data/verification/frankie-ingredient-search.png', fullPage: true, animations: 'disabled' });
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth + 1)).toBeTruthy();
});

test('organizer can open the exact pinned route, see private solutions, and browse builder steps', async ({ page }, testInfo) => {
  const dashboard = {
    hunts: [{ id: definition.id, title: definition.title, version: 1, status: 'live', definition }],
    drafts: [], help: [], photos: [], example: definition,
    templates: [{ id: 'frankie-code-hunt', title: 'Frankie challenge', description: 'Eight quick shop stages.', definition }],
    teams: [{
      id: state.teamId, name: 'Wrap Raiders', huntId: definition.id, version: 1, isPreview: false,
      lastActivity: '2026-09-22T10:00:10.000Z', view, ledger: state.ledger, events: state.events,
      checkpoints: state.checkpoints, definition,
    }],
  };
  await page.route('**/api/v2/admin', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(dashboard) }));
  await page.goto('/v2/admin');

  await page.getByRole('button', { name: /^Live control/ }).click();
  const team = page.locator('article').filter({ has: page.getByRole('heading', { name: 'Wrap Raiders', exact: true }) });
  await team.getByText('Route & private solutions', { exact: true }).click();
  await expect(team.getByText('TEAM IS HERE', { exact: true })).toBeVisible();
  await expect(team.getByText(/Accepted answers: frankie · a frankie · frankie roll/)).toBeVisible();
  if (testInfo.project.name === 'android-chrome') await page.screenshot({ path: '.data/verification/frankie-code-hunt-organizer.png', fullPage: true });
  await testInfo.attach('Frankie organizer mobile', { body: await page.screenshot({ fullPage: true }), contentType: 'image/png' });

  await page.getByRole('button', { name: 'Design', exact: true }).click();
  await page.getByRole('button', { name: /Frankie challenge/ }).click();
  await expect(page.getByRole('button', { name: 'Previous step', exact: true })).toBeDisabled();
  await expect(page.getByLabel('Accepted answers — one per line', { exact: true })).toHaveValue('frankie\na frankie\nfrankie roll\nroll\na roll\nwrap\na wrap\nkathi roll');
  await page.getByRole('button', { name: 'Next step', exact: true }).click();
  await page.getByRole('button', { name: 'Previous step', exact: true }).click();
  await expect(page.getByLabel('Accepted answers — one per line', { exact: true })).toHaveValue('frankie\na frankie\nfrankie roll\nroll\na roll\nwrap\na wrap\nkathi roll');
  expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth + 1)).toBeTruthy();
});
