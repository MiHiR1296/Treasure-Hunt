...
export default function FlowEditor({ value, hunt, onChange }: { value: CheckpointDefinition; hunt: HuntDefinition; onChange: (value: CheckpointDefinition) => void }) {
  ...
  const selectedIndex = value.flow.nodes.findIndex(node => node.id === selected?.id);
  ...
  return <section className="space-y-5">
    ...
    <div className="grid gap-5 xl:grid-cols-[minmax(0,1fr)_minmax(20rem,1fr)]">
      <div className="space-y-2"><div className="flex flex-wrap items-center justify-between gap-2"><p className="text-xs leading-5 text-slate-500">Select a card to edit it. Use its connection button to draw a route to another card. Arrows show play order.</p><div className="flex gap-2"><button type="button" className={buttonClass} disabled={Boolean(connecting) || selectedIndex <= 0} onClick={() => { const previous = value.flow.nodes[selectedIndex - 1]; if (previous) setSelectedId(previous.id); setRemoving(false); }}>Previous step</button><button type="button" className={buttonClass} disabled={Boolean(connecting) || selectedIndex < 0 || selectedIndex >= value.flow.nodes.length - 1} onClick={() => { const next = value.flow.nodes[selectedIndex + 1]; if (next) setSelectedId(next.id); setRemoving(false); }}>Next step</button></div></div><FlowCanvas checkpoint={value} selectedId={selected?.id || ''} connectingFrom={connecting?.sourceId} onSelect={select} /></div>
