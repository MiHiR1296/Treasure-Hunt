...
export default function HintPanel({ teamId, checkpointId, hints, disabled, send, notice, clearNotice }: {
  ...
  return (
    <section aria-label="Hints" className="space-y-3 border-t border-stone-200 pt-5">
      {hints.map((hint) => {
        const costLabel = hint.cost === 0 ? 'Free' : `${hint.cost} points`;
        ...
        {hint.status === 'available' && (confirmId === hint.id ? <div className="space-y-3">
          ...
          <button type="button" aria-label={`Reveal hint: ${hint.title} (${costLabel})`} disabled={disabled} className={primaryButton} onClick={async () => {
            const feedback = await send({ type: 'use_hint', checkpointId, hintId: hint.id });
            if (feedback) setConfirmId(null);
          }}>Reveal {hint.title}{hint.cost > 0 ? ` · ${hint.cost} points` : ' · Free'}</button>
        ...
        </div> : <button type="button" aria-label={`Choose hint: ${hint.title} (${costLabel})`} disabled={disabled} onClick={() => setConfirmId(hint.id)} className="hunt-action inline-flex min-h-11 items-center gap-2 px-1 text-sm font-semibold text-emerald-900 disabled:opacity-50"><span aria-hidden="true">?</span><span>Use this hint</span></button>)}
      </div>})}
    </section>
  );
}
